// app/deepseek/api/osm/buildings/route.ts
...
export async function GET(request: NextRequest) {
  ...
  // console.log("OSM API: Received bounds:", { north, south, east, west });

  // Validate bounds
  if (isNaN(north) || isNaN(south) || isNaN(east) || isNaN(west)) {
    // console.log("OSM API: Invalid bounds parameters");
    return new Response(
  ...
  if (north <= south || east <= west) {
    // console.log("OSM API: Invalid bounds order");
  ...
  if (latKm > maxKm || lonKm > maxKm) {
    // console.log("OSM Buildings API: Bounds area too large:", {
    //   latKm,
    //   lonKm,
    //   maxKm,
    //   latDiff,
  ...
  // Calculate a buffer around the requested bounds to ensure buildings aren't truncated
  // Convert approximate meters to degrees (1 degree ~ 111km at equator)
  const bufferInDegrees = 0.01; // ~1110 meters buffer (same as rivers and land)
  ...
  // Overpass API query for buildings in the buffered bounds
  const query = `
    [out:json][timeout:25];
    (
      way["building"](${bufferedSouth},${bufferedWest},${bufferedNorth},${bufferedEast});
    );
    out body;
    >;
    out skel qt;
  ...
  // console.log("OSM API: Sending query to Overpass API:", query);
  // console.log(
  //   "OSM API: Query bounds - south:",
  //   south,
  //   "west:",
  //   west,
  //   "north:",
  ...
  try {
    ...
    // console.log(
    //   "OSM API: Received response from Overpass API with status:",
    //   response.status
    // );

    if (!response.ok) {
      console.error(
        "OSM API: Overpass API returned error status:",
        response.status
      );
      const errorText = await response.text();
      console.error("OSM API: Error response from Overpass API:", errorText);
      throw new Error(
    ...
    console.log("OSM API: Returning", data.elements?.length || 0, "elements");
    ...
    console.error("OSM API: Error fetching OSM data:", error);
    if (error instanceof TypeError)
      return new Response(
        JSON.stringify({ error: "Failed to fetch OSM data: " + error.message }),
        {
          status: 500,
          headers: { "Content-Type": "application/json" },
        }
